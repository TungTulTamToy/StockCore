...
namespace StockCore.Factory.DB
{
    public class DBOperationStateRepoFactory : BaseFactory<string,IGetByKeyRepo<OperationState,string>>
    {
        ...
        private IGetByKeyRepo<OperationState, string> loadMonitoringDecorator(Tracer tracer, IGetByKeyRepo<OperationState, string> inner, Module module)
        {
            if (module.IsMonitoringActive())
            {
                inner = new MonGetByKeyRepoDec<string, OperationState>(
                    inner,
                    ValidationHelper.ValidateStringWithNotActivateOnly(1002105, "Quote"),
                    MONPROCESSERRID,
                    MONOUTERERRID,
                    module.Monitoring,
                    logger,
